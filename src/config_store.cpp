...
{
  ref_node_handle.get_parameter_or<bool>       ("odometry.enable"   , odom_enable_          , false);
  ref_node_handle.get_parameter_or<bool>       ("odometry.publishTf", odom_publish_tf_      , false);
...
{
  ref_node_handle.get_parameter_or<bool>("confWithRos", configure_through_ros_, false);
...
{
  ref_node_handle.get_parameter_or<double>("sensorParameters.initLat", init_condition_conf_.latitude, 48.419727);
  ref_node_handle.get_parameter_or<double>("sensorParameters.initLong", init_condition_conf_.longitude, -4.472119);
...
void ConfigStore::loadImuAlignementParameters(const rclcpp::Node& ref_node_handle)
{
  ...
  ref_node_handle.get_parameter_or<float>("imuAlignementLeverArm.misRoll", sensor_alignement_info_.misRoll, 0.0f);
  ref_node_handle.get_parameter_or<float>("imuAlignementLeverArm.misPitch", sensor_alignement_info_.misPitch, 0.0f);
  ref_node_handle.get_parameter_or<float>("imuAlignementLeverArm.misYaw", sensor_alignement_info_.misYaw, 0.0f);

  float sensor_lever_arm[3];
  ref_node_handle.get_parameter_or<float>("imuAlignementLeverArm.leverArmX", sensor_lever_arm[0], 0.0f);
  ref_node_handle.get_parameter_or<float>("imuAlignementLeverArm.leverArmY", sensor_lever_arm[1], 0.0f);
  ref_node_handle.get_parameter_or<float>("imuAlignementLeverArm.leverArmZ", sensor_lever_arm[2], 0.0f);

  sensor_lever_arm_ = SbgVector3<float>(sensor_lever_arm, 3);
}
...
void ConfigStore::loadGnssParameters(const rclcpp::Node& ref_node_handle)
{
  ...
  ref_node_handle.get_parameter_or<float>("gnss.primaryLeverArmZ", gnss_installation_.leverArmPrimary[2], 0.0f);
  ref_node_handle.get_parameter_or<bool>("gnss.primaryLeverPrecise", gnss_installation_.leverArmPrimaryPrecise, true);
  ref_node_handle.get_parameter_or<float>("gnss.secondaryLeverArmX", gnss_installation_.leverArmSecondary[0], 0.0f);
  ref_node_handle.get_parameter_or<float>("gnss.secondaryLeverArmY", gnss_installation_.leverArmSecondary[1], 0.0f);
  ref_node_handle.get_parameter_or<float>("gnss.secondaryLeverArmZ", gnss_installation_.leverArmSecondary[2], 0.0f);
...
{
  ref_node_handle.get_parameter_or<float>("odom.gain", odometer_conf_.gain, 4800.0f);
  ref_node_handle.get_parameter_or<bool>("odom.direction", odometer_conf_.reverseMode, false);

  float array_odometer_lever_arm[3];
  ref_node_handle.get_parameter_or<float>("odom.leverArmX", array_odometer_lever_arm[0], 0.0f);
  ref_node_handle.get_parameter_or<float>("odom.leverArmY", array_odometer_lever_arm[1], 0.0f);
  ref_node_handle.get_parameter_or<float>("odom.leverArmZ", array_odometer_lever_arm[2], 0.0f);

  odometer_lever_arm_ = SbgVector3<float>(array_odometer_lever_arm, 3);
...
{
  ref_node_handle.get_parameter_or<bool>("output.use_enu", use_enu_, false);

  if (use_enu_)
  {
    ref_node_handle.get_parameter_or<std::string>("output.frame_id", frame_id_, "imu_link");
  }
  else
  {
...
const sbg::SbgVector3<float> &ConfigStore::getSensorLeverArm() const
{
...
const sbg::SbgVector3<float> &ConfigStore::getOdometerLeverArm() const
{
  return odometer_lever_arm_;
}
